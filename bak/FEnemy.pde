class BasicEnemy extends Sprite {
  ...
  public void tickAttack(){
    if (!globalTimer.timerDone(this.staticHash)) {return;}
    ...
    switch (phase) {
      ...
      case 4:
        ...
        ((Game) scene).getAsteroids().add(new BasicAsteroid((int)(xCenter+Math.cos(PI*4/bnum*(subphase+1))*5*subphase), (int)(yCenter+Math.sin(PI*4/bnum*(subphase+1))*5*subphase), 50, 50, new int[]{255,255,255}, 5));
        ((Game) scene).getAsteroids().get(ilen).turn(360*2*((double)subphase/bnum)-90);
